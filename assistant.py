...
async def main():
    ...
    if use_azure_openai:
        ...
        agent = await OpenAIAssistantAgent.create(
            kernel=kernel, service_id=service_id, name=HOST_NAME, instructions=HOST_INSTRUCTIONS, ai_model_id="gpt-4o-mini"
            # kernel=kernel, service_id=service_id, name=HOST_NAME, instructions=HOST_INSTRUCTIONS, client=openAIClient, ai_model_id="phi3" #fail, ollama does not support assistant API yet
        )
    ...
    try:
        ...
        await invoke_agent(agent, thread_id=thread_id, input="How much is the Chai Tea?")
